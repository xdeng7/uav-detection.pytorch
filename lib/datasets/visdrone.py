...
class visdrone(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        imdb.__init__(self, 'visdrone_' + year + '_' + image_set)
        self._year = year
        ...
        self.config = {'cleanup': True,
                       'use_salt': True,
                       'use_diff': True,   # CHANGED FROM false
                       'matlab_eval': False,
                       'rpn_file': None,
                       'min_size': 2}

        assert os.path.exists(self._devkit_path), \
            'VisDrone_devkit path does not exist: {}'.format(self._devkit_path)
    ...
    def set_use_restarting(self, use_restarting):
        self._use_restarting = use_restarting
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'VisDrone_devkit' + self._year)
    ...
    def _get_voc_results_file_template(self):
        ...
        filedir = os.path.join(self._devkit_path, 'results', 'VisDrone' + self._year, isRestarting(self._use_restarting), 'altitude={}_angle={}_weather={}'.format(str(self._gamma_altitude), str(self._gamma_angle), str(self._gamma_weather)), str(self._epoch), str(self._ckpt))
        print("saving to:", filedir, "************************************************")
        ...
        for cls_ind, cls in enumerate(self.classes):
            ...
            with open(os.path.join(path, filename), 'wt') as f:
                for im_ind, index in enumerate(self.image_index):
                    ...
                    if dets == []:
                        continue
                    # the VOCdevkit expects 1-based indices, visdrone expects 0 based indices
    ...
    def _do_python_eval(self, path, output_dir='output'):
        ...
        print('VisDrone07 metric? ' + ('Yes' if use_07_metric else 'No'))
